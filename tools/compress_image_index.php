...
while(!$stop) {
  ...
  if($result === false) { die("Q1 ErrorMsg: ".$global['db']->ErrorMsg()); }
  if($result == null || $result->EOF) { $stop = true; }
  ...
  if(!$stop && ((int)$b != (int)($a + 1))) {
    ...
    ";
    $result = $global['db']->Execute($q);
    if($result === false) { die("Q2 ErrorMsg: ".$global['db']->ErrorMsg()); }
    echo "moved $b to ".($a+1)."\n";
  }
  $start++;
}
echo "Next image_id value: ".$start."\n";
$q = "UPDATE  image_seq SET `id` = '".(int)$start."';";
$result = $global['db']->Execute($q);
if($result === false) { die("Q3 ErrorMsg: ".$global['db']->ErrorMsg()); }
$q = "ALTER TABLE image_seq AUTO_INCREMENT = ".$start."; ";
$result = $global['db']->Execute($q);
if($result === false) { die("Q4 ErrorMsg: ".$global['db']->ErrorMsg()); }
