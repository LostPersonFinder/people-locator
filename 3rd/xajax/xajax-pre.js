function Xajax() {
  if (xajaxDebug) this.DebugMessage = function(text) { console.error("xajax debug: " + text) };
  ...
  this.call = function(sFunction, aArgs, sRequestType) {
    ...
    switch(xajaxRequestType) {
      ...
      break;
      default:
        console.error("Illegal request type: " + xajaxRequestType);
        return false;
      break;
    }
    ...
    if (xajaxRequestType == xajaxDefinedPost) {
      try {
        ...
        console.error("Your browser does not appear to support asynchronous requests using POST.");
        return false;
      }
    }
    r.onreadystatechange = function() {
      ...
      if(r.status==404) {
        // window.location = '?';
        toast('There was a 404 error...');
      } else if(r.status==200) {
        if (xajaxDebug && r.responseText.length < 1000) {
        ...
        if(r.responseXML) {
          ...
          toast('Error: the XML response from the server is invalid.');
  ...
  this.processResponse = function(xml) {
    ...
    if (xml == null) {
      console.log("Error: the XML response that was returned from the server cannot be processed.");
        ...
        if (xml.childNodes[i].childNodes.length > 1) {
          for (var j=0; j<xml.childNodes[i].childNodes.length; j++) {
            if (xml.childNodes[i].childNodes[j].nodeName == "s") {
              if (xml.childNodes[i].childNodes[j].firstChild)  search = xml.childNodes[i].childNodes[j].firstChild.nodeValue;
            }
            if (xml.childNodes[i].childNodes[j].nodeName == "r") {
              if (xml.childNodes[i].childNodes[j].firstChild) data = xml.childNodes[i].childNodes[j].firstChild.data;
            }
          }
        }  else if(xml.childNodes[i].firstChild) data = xml.childNodes[i].firstChild.nodeValue; else data = "";
        var objElement = this.$(id);
        try {
          if (cmd=="al") {
            console.error('base64>>' + btoa(data));
          }
          if (cmd=="js") {
            eval(data);
          }
